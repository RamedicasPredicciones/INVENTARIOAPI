...
def procesar_alternativas(faltantes_df, inventario_api_df):
    ...
    alternativas_disponibles_df['embalajeart'] = pd.to_numeric(alternativas_disponibles_df['embalajeart'], errors='coerce').fillna(0)
    alternativas_disponibles_df['embalaje_alternativa'] = pd.to_numeric(alternativas_disponibles_df['embalaje_alternativa'], errors='coerce').fillna(0)

    # Agregar la columna 'cantidad_necesaria'
    alternativas_disponibles_df['cantidad_necesaria'] = alternativas_disponibles_df.apply(
        lambda row: (row['embalajeart'] / row['embalaje_alternativa']) if row['embalaje_alternativa'] > 0 else 0,
        axis=1
    )
...
if uploaded_file:
    ...
    if not alternativas_disponibles_df.empty:
        ...
        st.dataframe(alternativas_filtradas_df[['codart', 'opcionart', 'codart_alternativa', 'opcion', 'nomart', 'carta', 'descontinuado', 'cantidad_necesaria']])

        if not alternativas_filtradas_df.empty:
            excel_file = generar_excel(alternativas_filtradas_df[['codart', 'opcionart', 'codart_alternativa', 'opcion', 'nomart', 'carta', 'descontinuado', 'cantidad_necesaria']])
            st.download_button(
